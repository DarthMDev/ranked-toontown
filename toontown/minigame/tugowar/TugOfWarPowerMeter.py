...
class TugOfWarPowerMeter(DirectFrame):
    ...
    def __init__(self, size, label = None):
        ...
        self.resetFrameSize()
        if label == None:
            label = TTLocalizer.TugOfWarPowerMeterLabel
        self.powerText = DirectLabel(self, relief=None, text=label, text_scale=TTLocalizer.MPMpowerText, pos=(0.01, 0.0, 0.29))
        self.tooSlow = DirectLabel(parent=self, relief=None, text=TTLocalizer.TugOfWarPowerMeterTooSlow, scale=TTLocalizer.MPMtooSlow, pos=(-.15, 0, 0.05), color=LVector4f(0.1, 0.3, 0.6, 1))
        self.tooFast = DirectLabel(parent=self, relief=None, text=TTLocalizer.TugOfWarPowerMeterTooFast, scale=TTLocalizer.MPMtooFast, pos=(0.15, 0, 0.05), color=LVector4f(0.1, 0.3, 0.6, 1))
        self.tooSlow.hide()
        self.tooFast.hide()
        self.largeGauge = []
        self.gaugeSize = size

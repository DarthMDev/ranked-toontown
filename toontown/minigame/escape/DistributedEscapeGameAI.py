...
class DistributedEscapeGameAI(DistributedMinigameAI):
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedEscapeGameAI')

    def __init__(self, air, minigameId):
        try:
            self.DistributedEscapeGameAI_initialized
        except:
            self.DistributedEscapeGame_initialized = 1
            DistributedMinigameAI.__init__(self, air, minigameId)
            self.gameFSM = ClassicFSM.ClassicFSM('DistributedEscapeGameAI', [State.State('inactive', self.enterInactive, self.exitInactive, ['play']), State.State('play', self.enterPlay, self.exitPlay, ['cleanup']), State.State('cleanup', self.enterCleanup, self.exitCleanup, ['inactive'])], 'inactive', 'inactive')
    ...
    def claimTreasure(self, sectionIndex, treasureIndex):
        ...
        if sectionIndex < 0 or sectionIndex >= len(self.sectionsSelected):
            self.air.writeServerEvent('warning', sectionIndex, 'EscapeAI.claimTreasure sectionIndex out of range.')
            return
        if treasureIndex < 0 or treasureIndex >= len(self.treasureTakenTable[sectionIndex]):
            self.notify.warning('Treasure %s: EscapeAI.claimTreasure treasureIndex out of range.' % treasureIndex)
            self.air.writeServerEvent('warning', treasureIndex, 'EscapeAI.claimTreasure treasureIndex out of range.')
            return
    ...
    def claimEnemyShot(self, sectionIndex, enemyIndex):
        ...
        if sectionIndex < 0 or sectionIndex >= len(self.sectionsSelected):
            self.air.writeServerEvent('warning', sectionIndex, 'EscapeAI.claimEnemyShot sectionIndex out of range.')
            return
    ...
    def toonVictory(self, avId, timestamp):
        if self.gameFSM.getCurrentState() == None or self.gameFSM.getCurrentState().getName() != 'play':
            msg = 'EscapeAI.toonVictory not in play state!'

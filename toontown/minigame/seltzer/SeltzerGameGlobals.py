# A file to put all seltzer game settings in one place for easy adjustment
...
# Colors of the countdown number right before a seltzer round starts.
...
# Ruleset
# Instance attached to seltzer game instances, so we can easily modify stuff dynamically
class SeltzerGameRuleset:
    """Ruleset for the seltzer game. Can be extended with modifiers in the future."""
    ...
    def __init__(self):
        ...
        # BOSS SETTINGS
        self.CEO_MAX_HP = 500  # How much HP should the CEO have?
        self.CEO_STUN_THRESHOLD = 24
        self.CEO_STUN_HIT_LIMIT = 5
        self.NUM_TABLES = 13
        self.NUM_GOLF_SPOTS = 4
        self.REORG_THRESHOLD = 0.66
        self.DOWNSIZE_THRESHOLD = 0.33
        self.SELTZER_DAMAGE_VALUES = [6, 12, 24]
        self.FOOD_HEAL_VALUES = [1, 2, 3, 4]
        self.TABLE_UNFLATTEN_TIME = 9
    ...
    # THE ATTRIBUTE INTO THIS LIST BELOW, AND A PARAMETER FOR IT IN THE DC FILE IN THE SeltzerGameRuleset STRUCT
    def asStruct(self):
        return [
            self.TIMER_MODE,
    ...
    def fromStruct(cls, attrs):
        ...
        if len(attrs) >= 12:
            ...
            rulesetInstance.CEO_MAX_HP = attrs[2]
...
# Modifier Base Class
# Instance attached to seltzer game instances, so we can easily modify stuff dynamically
class SeltzerGameModifierBase(object):
    """Base class for all seltzer game modifiers."""
...
# Modifiers
class ModifierFirstToXWins(SeltzerGameModifierBase):
    """First to X Wins modifier for the seltzer game."""
    
    MODIFIER_ENUM = 0
    MODIFIER_TYPE = SeltzerGameModifierBase.SPECIAL
    
    TITLE_COLOR = SeltzerGameModifierBase.DARK_PURPLE
    DESCRIPTION_COLOR = SeltzerGameModifierBase.PURPLE
...
for subclass in SeltzerGameModifierBase.__subclasses__():
    if hasattr(subclass, 'MODIFIER_ENUM') and subclass.MODIFIER_ENUM != -1:
        SeltzerGameModifierBase.MODIFIER_SUBCLASSES[subclass.MODIFIER_ENUM] = subclass

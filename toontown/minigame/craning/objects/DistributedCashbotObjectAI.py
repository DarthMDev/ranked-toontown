...
class DistributedCashbotObjectAI(DistributedSmoothNodeAI.DistributedSmoothNodeAI, FSM.FSM):
    ...
    def __init__(self, air, boss):
        ...
        FSM.FSM.__init__(self, 'DistributedCashbotObjectAI')
    ...
    def doFree(self, task):
        ...
        if not self.isDeleted():
            ...
            p = self.getPos()
            h = self.getH()
            # Preserve the Z position instead of resetting to 0
            # This allows objects to stay on platforms when entering Free state
            self.setPosHpr(p[0], p[1], p[2], h, 0, 0)
            self.sendUpdate('updateClientPositions', [p[0], p[1], p[2], h, 0, 0])
        return Task.done

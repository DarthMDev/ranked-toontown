...
class DistributedCashbotObject(DistributedSmoothNode.DistributedSmoothNode, FSM.FSM):

    """ This is an object that can be picked up an dropped in the
    final battle scene with the Cashbot CFO.  In particular, it's a
    safe or a goon.  """
    
    notify = DirectNotifyGlobal.directNotify.newCategory('DistributedCashbotObject')
    ...
    def __init__(self, cr):
        ...
        FSM.FSM.__init__(self, 'DistributedCashbotObject')
    ...
    def __hitPlatform(self, entry):
        ...
        if entry:
            ...
            if intoNodePath and not intoNodePath.isEmpty():
                ...
                # the DistributedCashbotFloatingPlatform's model node (named 'FloatingPlatform-{index}')
